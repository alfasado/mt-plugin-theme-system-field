package ThemeSystemField::Theme;
use strict;
use MT;
...
sub apply {
    ...
    for my $basename ( keys %$fields ) {
        $field_class->count( { basename => $basename } )
            and next;
...
sub template {
    ...
    my @fields = MT->model('field')->load( { blog_id => 0, } );
    return unless scalar @fields;
    my @list;
    my %checked_ids;
    if ($saved) {
        %checked_ids
            = map { $_ => 1 } @{ $saved->{system_fields_export_ids} };
    }
    for my $field (@fields) {
    ...
    my $component = MT->component( 'ThemeSystemField' );
...
sub export {
    ...
    my @fields;
    if ( defined $settings ) {
        my $ids = $settings->{system_fields_export_ids};
        if (@$ids) {
            @fields = MT->model('field')->load( { id => $ids, } );
        }
    }
    else {
        @fields = MT->model('field')->load( { blog_id => 0, } );
    }
    return unless scalar @fields;
    my %data = map {
        $_->basename => {
